  ...
  <div>
    ...
    >
      <div class="grid md:grid-cols-2 gap-4 my-4 p-4">
        ...
        <div class="w-full flex justify-end">
          <input
            ...
            class="border w-3/4 rounded outline-none p-3 focus:border-blue-400"
          />
        </div>
      </div>
      ...
      <table class="w-full items-center table-auto">
        ...
        <tbody
          v-if="student && filteredStudent.length > 0"
          class="text-middey-text overflow-x-auto"
        >
          <tr
            v-for="(item, idx) in filteredStudent"
...
export default {
  ...
  async mounted() {
    ...
    this.studentCount = this.student?.count
    this.loading = false
  },
  computed: {
    filteredStudent() {
      return this.student?.results.filter((candidate) => {
        return (
